-- Public adult trade onboarding for Chợ Neo.
-- Existing invitation records remain recoverable, but a first-time adult trade
-- member no longer needs an invitation to create a moderated member profile.
...
alter table public.cho_neo_member_profiles
  drop constraint if exists cho_neo_member_profiles_onboarding_source_check;

alter table public.cho_neo_member_profiles
  add constraint cho_neo_member_profiles_onboarding_source_check
  check (
    onboarding_source is null or onboarding_source in (
      'private_invitation',

-- Chợ Neo Guest Pass V1
-- Adds guest profiles and Supabase-user-owned room votes.
...
alter table public.cho_neo_room_votes
  add column if not exists voter_hash text null,
  add column if not exists voter_user_id uuid null references auth.users(id) on delete cascade,
  add column if not exists optional_reason text null,

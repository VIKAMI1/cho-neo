-- Fix the private invitation profile upsert after the onboarding migration.
-- Dynamic SQL keeps ON CONFLICT (user_id) in table-column scope instead of
-- colliding with the RETURNS TABLE output variable named user_id.
...
begin
  ...
  select invitation.*
    into invitation_row
  from public.cho_neo_member_invitations as invitation

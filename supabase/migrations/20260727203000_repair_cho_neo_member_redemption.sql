-- Repair Chợ Neo Social Member redemption after Guest Pass schema evolution.
-- Prior Preview/test databases may still have legacy Guest Pass artifacts on
-- cho_neo_member_profiles after cho_neo_guest_profiles was renamed.

do $$
begin
  if to_regclass('public.cho_neo_member_profiles') is not null
     and exists (
       select 1
    ...
    execute $sql$
      ...
      set membership_status = case
        when member.membership_status in ('suspended', 'rejected')
          then member.membership_status
        when member.status in ('banned', 'suspended')
          then 'suspended'
        when member.status = 'active'
          then coalesce(nullif(member.membership_status, 'pending'), 'pending')
...
alter table if exists public.cho_neo_member_profiles
  drop constraint if exists cho_neo_guest_profiles_status_check,
  drop constraint if exists cho_neo_guest_profiles_avatar_key_check;

drop trigger if exists set_cho_neo_guest_profiles_updated_at
  on public.cho_neo_member_profiles;

drop function if exists public.set_cho_neo_guest_profiles_updated_at();

drop index if exists public.cho_neo_guest_profiles_status_idx;

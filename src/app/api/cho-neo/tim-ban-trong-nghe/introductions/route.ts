...
export async function POST(request: Request) {
  ...
  const [{ data: profiles }, { data: memberProfiles }, { data: blocks, error: blockError }] = await Promise.all([
    supabase.from(CHO_NEO_MATCHING_PROFILE_TABLE).select("user_id, status, connection_intent").in("user_id", members).eq("status", "active"),
  ...
  const connectionIntents = (profiles ?? []).map((profile) => profile.connection_intent ?? "professional");
  if (connectionIntents.includes("personal") && connectionIntents.some((intent) => intent !== "personal")) {
    return NextResponse.json({ error: "Kết nối cá nhân chỉ mở khi cả hai cùng chọn điều đó." }, { status: 409 });
  }

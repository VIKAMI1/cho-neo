...
function getOpenAIGate(): ChoNeoOpenAIReservation {
  if (!isChoNeoOpenAIEnabled()) return { ok: false, reason: "openai-disabled" };
  if (!process.env.OPENAI_API_KEY?.trim()) return { ok: false, reason: "missing-api-key" };
  return { ok: true, reservationId: "" };
}

function createOpenAIIdempotencyKey(
  memberUserId: string,
  prayer: string,
  history: ReturnType<typeof cleanConversationHistory>,
) {
  const digest = createHash("sha256")
    .update(JSON.stringify({ memberUserId, prayer, history }), "utf8")
    .digest("hex");
  return `ong-dia:${digest}`;
}
...
> {
  ...
  if (reason === "global-limit" || reason === "global-token-limit") {
    return "fallback_openai_global_limit";
  }
...
export async function POST(request: Request) {
  ...
  let openAIUsageClient: OpenAIUsageClient | null = null;
  let openAIReservationId: string | null = null;
  let openAIFlightKey: string | null = null;
  let openAISucceeded = false;
  let openAIActualTokens = 0;
  ...
  if (provider === "openai") {
    const gate = getOpenAIGate();
    if (gate.ok === false) {
      const source = gate.reason === "missing-api-key"
        ? "fallback_no_api_key"
        : getOpenAICircuitBreakerSource(gate.reason);
      return createPrayerJson(
        ...
        "openai",
        {
          requestId,
          startedAt,
          status: source === "fallback_openai_member_limit"
            ? 429
            : source === "fallback_no_api_key"
              ? 200
              : 503,
    ...
    const serviceClient = createSupabaseServiceClient();
    const idempotencyKey = createOpenAIIdempotencyKey(member.userId, prayer, history);
    if (!serviceClient || !claimOpenAIUsageFlight(idempotencyKey)) {
      const reason: Exclude<ChoNeoOpenAIReservation, { ok: true }>["reason"] =
        serviceClient ? "duplicate-in-flight" : "usage-unavailable";
      const source = getOpenAICircuitBreakerSource(reason);
      return createPrayerJson(
        getFallbackForSource(source, fallback, prayer),
        source,
        "openai",
        { requestId, startedAt, status: 503 },
      );
    }

    const reservationResult = await reserveOpenAIUsage(
      serviceClient,
      member.userId,
      idempotencyKey,
    );
    const reservation: ChoNeoOpenAIReservation = reservationResult.allowed && reservationResult.reservation_id
      ? { ok: true, reservationId: reservationResult.reservation_id }
      : {
          ok: false,
          reason: reservationResult.reason === "global-limit" ||
            reservationResult.reason === "global-token-limit" ||
            reservationResult.reason === "member-limit"
            ? reservationResult.reason
            : reservationResult.reason === "idempotent"
              ? "idempotent-reservation"
              : reservationResult.reason === "invalid-usage-config"
                ? "invalid-usage-config"
                : "usage-unavailable",
        };
    if (!reservation.ok) {
    ...
    openAIUsageClient = serviceClient;
    openAIReservationId = reservation.reservationId;
    openAIFlightKey = idempotencyKey;
  }
  ...
  try {
    ...
    if (providerResult.telemetry) {
      providerTelemetry.push(providerResult.telemetry);
      if (provider === "openai") {
        openAIActualTokens = providerResult.telemetry.totalTokens ?? CHAT_MAX_TOKENS;
      }
    }
    ...
    if (openAIReservationId && openAIUsageClient) {
      const finalized = await finalizeOpenAIUsage(
        openAIUsageClient,
        openAIReservationId,
        openAISucceeded,
        openAIActualTokens,
      ...
      if (!finalized) {
        console.warn("[ong-dia-prayer] OpenAI usage finalization unavailable");
      }
    }
    if (openAIFlightKey) releaseOpenAIUsageFlight(openAIFlightKey);
  }
}

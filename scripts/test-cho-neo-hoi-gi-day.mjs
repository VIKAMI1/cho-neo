...
test("disabled AI persists and visibly returns the graceful fallback", () => {
  ...
  assert.match(ai, /if \(!gate\.allowed\) return fallback\(gate\.reason\);[\s\S]*const reservation = await reserveUsage/);
});
...
test("feedback is explicit, review-gated and cannot replace an answer", () => {
  ...
  assert.doesNotMatch(route, /cho_neo_answers[\s\S]*\.update\(/);
...
test("shared Supabase reservation is authoritative before provider fetch", () => {
  assert.match(ai, /reserve_cho_neo_openai_usage_v2/);
  assert.match(ai, /finalize_cho_neo_openai_usage/);
  assert.match(ai, /p_cooldown_seconds/);
  assert.match(ai, /p_monthly_token_limit/);
  assert.match(ai, /const reservation = await reserveUsage/);

...
const usagePath = path.join(repoRoot, "src/lib/cho-neo/openai-usage.ts");
...
const routeSource = fs
  ...
  .replace(
    ...
    `from ${JSON.stringify(pathToFileURL(usagePath).href)};`,
  );
...
function handleChoNeoSupabaseFetch(url, init = {}) {
  ...
  if (href.includes("/rest/v1/rpc/reserve_cho_neo_openai_usage_v2")) {
    const body = init.body ? JSON.parse(String(init.body)) : {};
    mock.rpcCalls.push({ name: "reserve_cho_neo_openai_usage_v2", body });
    if (mock.rpc === "error") {
  ...
  if (href.includes("/rest/v1/rpc/finalize_cho_neo_openai_usage")) {
    const body = init.body ? JSON.parse(String(init.body)) : {};
    mock.rpcCalls.push({ name: "finalize_cho_neo_openai_usage", body });
    return supabaseJson([{ ok: true, reason: body.p_success ? "succeeded" : "released" }]);
  }
...
function usageRpcCalls() {
  return choNeoSecurityMock?.rpcCalls ?? [];
}
...
test("OpenAI is never invoked for anonymous invalid and non-verified member requests", async () => {
  ...
  for (const { name, security, headers } of deniedCases) {
    ...
    assert.equal(usageRpcCalls().length, 0);
...
test("enabled OpenAI flag plus verified membership reaches provider adapter once", async () => {
  ...
  assert.deepEqual(
    usageRpcCalls().map((call) => call.name),
    ["reserve_cho_neo_openai_usage_v2", "finalize_cho_neo_openai_usage"],
  );
  assert.equal(usageRpcCalls()[1].body.p_success, true);
  assert.equal(usageRpcCalls()[1].body.p_actual_tokens, 255);
});

test("provider failures finalize the shared reservation as failed", async () => {
  process.env.ONG_DIA_PROVIDER = "openai";
  process.env.OPENAI_API_KEY = "test-openai-key";
  const cases = [
    { name: "429", mock: { status: 429 } },
    { name: "500", mock: { status: 500 } },
    { name: "malformed", mock: { content: JSON.stringify({ loiOngDia: "" }) } },
    { name: "timeout", mock: { delayMs: 1200 }, timeout: "1000" },
  ];

  for (const failureCase of cases) {
    configureChoNeoSecurity({ openAIEnabled: true, rpc: "allowed" });
    if (failureCase.timeout) process.env.ONG_DIA_AI_TIMEOUT_MS = failureCase.timeout;
    else delete process.env.ONG_DIA_AI_TIMEOUT_MS;
    mockOpenAI(failureCase.mock);
    await callPrayer({ prayer: `Con cần Ông nghe chuyện ${failureCase.name}.` });

    const calls = usageRpcCalls();
    assert.equal(calls[0].name, "reserve_cho_neo_openai_usage_v2");
    assert.equal(calls[1].name, "finalize_cho_neo_openai_usage");
    assert.equal(calls[1].body.p_success, false);
  }
});

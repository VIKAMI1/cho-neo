...
- Health check: `GET /api/health` returns only minimal service status. It must
  never return environment variable values or secrets.
...
- Shared Quán Tám memory: production should set `NEXT_PUBLIC_SUPABASE_URL` and
  ...
  an authenticated owner/admin session and `SUPABASE_SERVICE_ROLE_KEY`.
